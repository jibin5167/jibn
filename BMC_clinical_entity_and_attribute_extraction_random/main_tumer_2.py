...
from LSTM_for_tumer.datas import read_corpus, read_dictionary, tag2label, random_embedding, read_testdata, output_data
        ...
        if os.path.exists(args.data_source):
            ...
            for f in filenames:
                ...
                if len(predict)==2:
                    ...
                    # if (tumer1+'MT') in EMR and not (tumer2+'MT') in EMR and tumer2 in predict:
                    #     predict.remove(tumer2)
                    # if (tumer2+'MT') in EMR and not (tumer1+'MT') in EMR and tumer1 in predict:
                    #     predict.remove(tumer1)
                ...
                #     tumer2 = predict[1]
                #     tumer3 = predict[2]
                #     if (tumer1+'癌') in EMR and not (tumer2+'癌') in EMR and not (tumer3+'癌') in EMR:
                #         if tumer2 in predict:
                #             predict.remove(tumer2)
                #         if tumer3 in predict:
                #             predict.remove(tumer3)
                #     if not (tumer1+'癌') in EMR and (tumer2+'癌') in EMR and not (tumer3+'癌') in EMR:
                #         if tumer1 in predict:
                #             predict.remove(tumer1)
                #         if tumer3 in predict:
                #             predict.remove(tumer3)
                #     if not (tumer1+'癌') in EMR and not (tumer2+'癌') in EMR and (tumer3+'癌') in EMR:
                #         if tumer1 in predict:
                #             predict.remove(tumer1)
                #         if tumer2 in predict:
                #             predict.remove(tumer2)
                ...
                #     tumer2 = predict[1]
                #     tumer3 = predict[2]
                #     if (tumer1+'癌') in EMR and (tumer2+'癌') in EMR and not (tumer3+'癌') in EMR:
                #         if tumer3 in predict:
                #             predict.remove(tumer3)
                #     if not (tumer1+'癌') in EMR and (tumer2+'癌') in EMR and (tumer3+'癌') in EMR:
                #         if tumer1 in predict:
                #             predict.remove(tumer1)
                #     if (tumer1+'癌') in EMR and not (tumer2+'癌') in EMR and (tumer3+'癌') in EMR:
                #         if tumer2 in predict:
                #             predict.remove(tumer2)
